...
bool AKCalibr::Process(const std::vector<Mat>& imgList)
{
    ...
    {        
		...
		//使用findCirclesGrid函数来检测图片中的圆网格//输入图像，角点行列数，角点
	...
	//相机标定，获取标定图的内角点图像坐标之后，使用calibration函数计算相机内参和外参系数
    ...
    if (ret && mConfig.showUndistorsed)
    {
        ...
        for (size_t i = 0; i < imgList.size(); ++i)
        {
			...
			remap(view, rview, map1, map2, INTER_LANCZOS4);
			//////////////////////////
			Mat newview; 
			imgList[i].copyTo(newview);
			imshow("compare",newview);
			/////////////////////////
